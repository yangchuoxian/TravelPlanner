...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        if userCredential != nil {  // username and loginToken already stored in keychain, automatic login now
            let currentUserId = userCredential!["currentUserId"]
            let userInfoResponseData = Toolbox.syncHttpGetFromURL("\(URLGetUserInfo)?id=\(currentUserId!)")
            if userInfoResponseData != nil {    // automatic login succeeded
